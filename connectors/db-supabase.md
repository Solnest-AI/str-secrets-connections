...
Three values end up in `.env`. You supply one (the access token). Claude fills the other two (the project ref and the database password) when it creates the project. Do not type those two by hand. If the kit joins a project that already exists (an existing Revenue Manager registration, or a project you chose to reuse, both in section 3), only the ref is filled and the password line stays blank; the skills do not need it.
...
2. The dialog that opens is a two-step scoped wizard ("Step 1 of 2 · Configure"). Do not fill it in. Under Resource access there is a line: "Need a token with full access to your account? Create legacy token". Click **Create legacy token**. Supabase's warning on that screen is real: "Access tokens can be used to control your whole account. Be careful when sharing your tokens." Their API reference says the same: "PATs carry the same privileges as your user account, so be sure to keep it secret." That is why it only ever goes into `.env`.
3. **Name:** `STR Secrets`. **Expires in:** open the dropdown (24 hours / 7 days / 30 days / 90 days / Custom) and pick **Custom**, then the furthest date the picker allows. It caps at exactly one year; on 2026-09-21 the picker offered 21 Sep 2027. Never leave it on 7 days. Non-expiring tokens can no longer be created, for either kind of token. Legacy and scoped differ in permissions, not expiry.
...
**When it expires:** make a new one the same way, paste it into `.env`, then re-run the Register block below and restart Claude Code. The server keeps its own copy of the token in `~/.claude.json`; pasting into `.env` alone does not reach it. Then say "Check my connections".
...
**Already have `supabase-revenue-manager` from the Revenue Manager kit?** The checker never shows a server's command line, so Claude pulls the ref on its own with `claude mcp list 2>/dev/null | sed -n 's/^supabase-revenue-manager: .*--project-ref=\([a-z]\{20\}\).*/\1/p'`, which prints the 20-letter ref or nothing at all (never the rest of the list; the token is in the env, which that list never shows). If a ref comes back and `SUPABASE_PROJECT_REF` in `.env` is blank, Claude writes that ref with `env_set_if_blank "$BUNDLE/.env" SUPABASE_PROJECT_REF <ref>` and skips the project step: your Revenue Manager tables already live in that project and the summit skills join it. Only run the project step when no ref is registered yet.

**Register** the server. Source `.env` first so the shell can read both values, then:
```bash
set -a; . "$BUNDLE/.env"; set +a
claude mcp remove supabase-revenue-manager -s user >/dev/null 2>&1 || true
claude mcp add --transport stdio supabase-revenue-manager --scope user --env SUPABASE_ACCESS_TOKEN="$SUPABASE_ACCESS_TOKEN" -- npx -y @supabase/mcp-server-supabase@latest --project-ref="$SUPABASE_PROJECT_REF" >/dev/null 2>&1 && echo "supabase-revenue-manager registered ✅"
echo supabase-revenue-manager >> "$BUNDLE/.cache/needs-restart"
```
No `--read-only` flag. The skills apply migrations through this server, so it needs write access. The token is stored as the server's environment in `~/.claude.json`, which is why the kit never shows you raw `claude mcp list` output and never asks you to print the config.
...
Supabase's documented Claude Code path is a hosted server with a browser login. For reference only, their exact line:
```bash
...
```
then, in their words, "In a regular terminal (not the IDE extension) run: claude /mcp Select the "supabase" server, then "Authenticate"". A browser window opens where you log in to Supabase and grant access to your organization. Their URL also takes `read_only=true`, `project_ref=<id>` "(disables account tools)", and `features=`.
...
After the token passes, the row also needs `SUPABASE_PROJECT_REF` filled (the project step does that, or the Revenue Manager check just above the Register block) and the server registered. After you restart Claude Code, Claude asks the server for `list_tables`. An empty result is a pass; the skills create their tables on first run. A tool error mentioning permissions or `403` means the token is scoped too tightly: it passed the token check but cannot write. Make a legacy token, or Organization plus Full access, paste it into `.env`, re-run the Register block, restart.

## 6. Troubleshooting
- **`CONNECTION_CLOSED` the first time the server connects:** an npm peer dependency problem in the published package. It declares `zod` and `@modelcontextprotocol/server` as peer dependencies (checked on npm 2026-09-21), and Claude Code issue #95435 tracks the failure. Install the three locally and re-register pointing at the local binary:
  ```bash
  cd "$BUNDLE" && npm install --silent @supabase/mcp-server-supabase @modelcontextprotocol/server zod
  set -a; . "$BUNDLE/.env"; set +a
  claude mcp remove supabase-revenue-manager -s user >/dev/null 2>&1 || true
  claude mcp add --transport stdio supabase-revenue-manager --scope user --env SUPABASE_ACCESS_TOKEN="$SUPABASE_ACCESS_TOKEN" -- "$BUNDLE/node_modules/.bin/mcp-server-supabase" --project-ref="$SUPABASE_PROJECT_REF" >/dev/null 2>&1 && echo "supabase-revenue-manager registered ✅"
  echo supabase-revenue-manager >> "$BUNDLE/.cache/needs-restart"
  ```
  **Windows note:** that binary path is handed to Claude Code, a native Windows process, so it must be the `C:\...` form, and the `.bin` shim is a Unix script. Register the package's real entry through `node` instead, converted with `cygpath -w`: `-- node "$(cygpath -w "$BUNDLE/node_modules/@supabase/mcp-server-supabase/dist/cli.js")" --project-ref="$SUPABASE_PROJECT_REF"` in place of the `.bin` path. (`dist/cli.js` is the package's declared `bin` entry, checked on npm 2026-09-21.)
- **Worked last week, 401 today:** the token expired. If you left the dropdown on 7 days, this is that. New token, Custom expiry a year out, paste it into `.env`, re-run the Register block in section 3, restart Claude Code, then "Check my connections".
...
- **Windows: `Failed to connect` after restart:** first check `npx -v` prints a version in Git Bash and that the `npm.ps1` execution-policy fix from `system-node.md` has been applied. If it still fails, Claude Code (a native Windows process) could not launch `npx` directly. Re-register through `cmd`:
  ```bash
  set -a; . "$BUNDLE/.env"; set +a
  claude mcp remove supabase-revenue-manager -s user >/dev/null 2>&1 || true
  claude mcp add --transport stdio supabase-revenue-manager --scope user --env SUPABASE_ACCESS_TOKEN="$SUPABASE_ACCESS_TOKEN" -- cmd //c npx -y @supabase/mcp-server-supabase@latest --project-ref="$SUPABASE_PROJECT_REF" >/dev/null 2>&1 && echo "supabase-revenue-manager registered ✅"
  echo supabase-revenue-manager >> "$BUNDLE/.cache/needs-restart"
  ```
  Restart Claude Code again. The Revenue Manager runner still finds `--project-ref=` in the args, so this shape keeps its contract.
