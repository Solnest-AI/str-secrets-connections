...
**Register** the server. This is the canonical register block; the troubleshooting fallbacks in section 6 refer back to it and only swap the one line that differs. Source `.env` first so the shell can read both values, then:
```bash
set -a; . "$BUNDLE/.env"; set +a
...
claude mcp add --transport stdio supabase-revenue-manager --scope user --env SUPABASE_ACCESS_TOKEN="$SUPABASE_ACCESS_TOKEN" -- npx -y @supabase/mcp-server-supabase@latest --project-ref="$SUPABASE_PROJECT_REF" >/dev/null 2>&1 && echo "supabase-revenue-manager registered ✅" || echo "supabase-revenue-manager failed ❌ (run the same line without the >/dev/null part to see why)"
...
## 6. Troubleshooting
- **`CONNECTION_CLOSED` the first time the server connects:** an npm peer dependency problem in the published package. It declares `zod` and `@modelcontextprotocol/server` as peer dependencies (checked on npm 2026-09-21), and Claude Code issue #95435 tracks the failure. Install the three locally, then re-run the Register block in section 3, but swap its `-- npx -y @supabase/mcp-server-supabase@latest` for `-- "$BUNDLE/node_modules/.bin/mcp-server-supabase"`:
  ```bash
