...
## 5. Verify
The checker reads Claude Code's own server status for `meta-ads` and shows one row, **Meta Ads MCP**:

- **Green**: Claude Code reports the server as `✔ Connected`. Move on to the real test.
- **Auth**: registered but not signed in yet. Run `/mcp` > meta-ads > **Authenticate** and finish the browser sign-in.
- **Restart**: registered this session and Claude Code has not been restarted. Restart, then say "Check my connections" again.
- **Missing**: not registered. Run the register block in section 4.
- **Pending / approve it in `/mcp`**: Claude Code is waiting for you to approve the server. Open `/mcp` and approve it.
...
- **Error saying no active ad account**: open https://adsmanager.facebook.com, confirm an ad account shows there, add a payment method if it asks (nothing runs without your say), then ask the Ad Library question again. If it still errors with a payment method on file, Meta wants more from the account than that (unverified what, possibly a confirmed payment). Stop there: the Ad Spy runs degraded without it and the rest of the kit is unaffected.
