...
```bash
if ( cd "$BUNDLE/mcp-servers/kie" && uv venv --quiet .venv && uv pip install --quiet -r requirements.txt --python .venv ) && [ -x "$BUNDLE/mcp-servers/kie/.venv/bin/python" ]; then
  echo "kie venv built ✅"
  claude mcp remove kie -s user >/dev/null 2>&1 || true
  if claude mcp add --transport stdio kie --scope user --env KIE_ENV_PATH="$BUNDLE/.env" -- "$BUNDLE/mcp-servers/kie/.venv/bin/python" "$BUNDLE/mcp-servers/kie/server.py" >/dev/null 2>&1; then echo "kie registered ✅"; echo kie >> "$BUNDLE/.cache/needs-restart"; else echo "kie register failed ❌. Run the claude mcp add line again without the >/dev/null 2>&1 part to see the error."; fi
else
...
```bash
if ( cd "$BUNDLE/mcp-servers/kie" && uv venv --quiet .venv && uv pip install --quiet -r requirements.txt --python .venv ) && [ -f "$BUNDLE/mcp-servers/kie/.venv/Scripts/python.exe" ]; then
  echo "kie venv built ✅"
  claude mcp remove kie -s user >/dev/null 2>&1 || true
  if claude mcp add --transport stdio kie --scope user --env KIE_ENV_PATH="$(cygpath -w "$BUNDLE/.env")" -- "$(cygpath -w "$BUNDLE/mcp-servers/kie/.venv/Scripts/python.exe")" "$(cygpath -w "$BUNDLE/mcp-servers/kie/server.py")" >/dev/null 2>&1; then echo "kie registered ✅"; echo kie >> "$BUNDLE/.cache/needs-restart"; else echo "kie register failed ❌. Run the claude mcp add line again without the >/dev/null 2>&1 part to see the error."; fi
else
  echo "kie venv build failed ❌. Nothing registered. Is uv installed? See connectors/system-python-uv.md, then run this block again."
fi
```
...
- **`kie` shows Failed after restart and the register line printed ✅ (any OS):** the venv was never built, so the Python path stored in the registration does not exist. `claude mcp add` does not check that. Re-run the register block in section 3 until it prints `kie venv built ✅` before `kie registered ✅`. If it prints `build failed`, uv is missing or the install could not download; see connectors/system-python-uv.md.
