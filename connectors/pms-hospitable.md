...
**Hospitable MCP (official)** row: until its live check has passed once, the row prints `🔎 Hospitable MCP (official)   add it under + > Connectors; Claude checks it live`, whether or not you have added it yet, and even if you used the fallback bearer token below (that path really does register in `~/.claude.json` and needs a restart, the row text just does not know to say so for this server). Once the live check passes, Claude records it in `.cache/live-ok` and the row shows ✅ with the date from then on; "recheck hospitable-official" runs it again.
