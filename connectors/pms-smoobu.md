...
- **Heads up on timing:** Smoobu is switching off the old single-header login. Smoobu's developer docs now say the old header is "deprecated, sunset October 31, 2026" (they moved it from September 25; read 2026-09-25). The old way dies about a month after the summit. The server Claude builds signs every request the new way from day one, so nothing breaks for you when the switch lands.
...
- **Everything worked, then 401 across the board on or after October 31, 2026:** the legacy `Api-Key` header got switched off and something is still sending it. The built server does not, so this means a stale build. Rebuild (`npm run build` in the server folder) and restart.
...
## 7. Sources
docs.smoobu.com (#hmac-authentication; legacy header sunset note), support.smoobu.com articles 360003170740 (create an API key) and 38810363709330, smoobu.com/en/pricing. All read 2026-09-21; sunset date re-read 2026-09-25 (now October 31, 2026).
