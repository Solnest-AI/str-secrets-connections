...
claude mcp remove guesty --scope user >/dev/null 2>&1; claude mcp add --transport stdio guesty --scope user -- node "$BUNDLE/mcp-servers/guesty/dist/index.js" >/dev/null 2>&1 && echo "guesty registered ✅" || echo "register failed ❌"
...
```bash
bash "$BUNDLE/fan-out-env.sh" >/dev/null && echo "fanned out ✅" || echo "fan-out failed ❌"
claude mcp remove guesty --scope user >/dev/null 2>&1; claude mcp add --transport stdio guesty --scope user -- node "$(cygpath -w "$BUNDLE/mcp-servers/guesty/dist/index.js")" >/dev/null 2>&1 && echo "guesty registered ✅" || echo "register failed ❌"
echo guesty >> "$BUNDLE/.cache/needs-restart"
```
...
Source the `.env` first so the shell can read the two values, then register:
```bash
set -a; . "$BUNDLE/.env"; set +a
claude mcp add --transport stdio guesty-official --scope user --env CLIENT_ID="$GUESTY_CLIENT_ID" --env CLIENT_SECRET="$GUESTY_CLIENT_SECRET" -- npx -y @guestyorg/sdk@0.2.0-alpha.1 mcp >/dev/null 2>&1 && echo "guesty-official registered ✅"
echo guesty-official >> "$BUNDLE/.cache/needs-restart"
```
The `@0.2.0-alpha.1` pin is deliberate. `@0.2.0` does not exist on npm (checked 2026-09-21: only `0.2.0-alpha` and `0.2.0-alpha.1` are published). Do not "clean it up." Guesty's docs show `npx -y @guestyorg/sdk mcp` with no version; the pin makes sure you get the newest published build, and the same one every time (unpinned, npm's `latest` tag hands you the older `0.2.0-alpha`). Guesty's MCP page lists Cursor, Claude Desktop, VS Code and Antigravity; it has no Claude Code section, so the `claude mcp add` line above is our translation of their config, not a line Guesty published or one we have run end to end.

Both values land in `~/.claude.json` as that server's environment. That is a second copy of your secret, which is one reason we never print `claude mcp list` output to you.

**Read this before you register it.** Guesty: "When using CLIENT_ID and CLIENT_SECRET, each restart requests a new token, which counts toward the 5-tokens-per-24-hours limit." The package keeps that token in memory only (checked in 0.2.0-alpha.1), so each of these spends one of today's five: every Claude Code start, in any folder (user scope means it starts everywhere); every `claude mcp list`, and the checker runs one each time you say "Check my connections"; every `/mcp` reconnect. WORKS spends one more only when `.cache/guesty.token` is missing or older than 23 hours. On setup day Claude restarts Claude Code twice and runs the checker after each, so with `guesty-official` registered you are at five before the summit starts. Default: skip Path B. The built `guesty` server does everything the summit needs and shares the cached token. If you want `guesty-official` anyway, register it on a day you are not setting anything else up, after the checker is green, and expect the `Guesty MCP (official, beta)` row to cost one slot per check. Claude: if the attendee skips Path B, that row stays ❌ on purpose. Say so once and move on.
...
**Windows note:** the only paths in this block (`$BUNDLE/.env`, the needs-restart file) are read by Git Bash itself, not handed to Claude Code, so no `cygpath -w` here. If `guesty-official` shows `Failed to connect` after the restart and `node -v` is 20 or newer, remove it (`claude mcp remove guesty-official -s user`) and register again with `-- cmd //c npx -y @guestyorg/sdk@0.2.0-alpha.1 mcp` in place of `-- npx -y @guestyorg/sdk@0.2.0-alpha.1 mcp` (double slash on purpose: Git Bash turns a lone `/c` into `C:/`). That is one more restart, so one more token slot; do it tomorrow if today's count is already at four.
...
The checker reads `claude mcp list` once per run and reports `guesty` and, if you did Path B, `guesty-official`. Do not run `claude mcp list` by hand to re-check: every run starts `guesty-official` and spends a token. `Connected` is what you want. `Failed to connect` on `guesty` right after a build usually means the build did not finish or `fan-out-env.sh` has not run since the build. `Failed` on `guesty-official` the first time is often the `npx` download running past the startup timeout; the next "Check my connections" retries it, and that retry costs a slot.
...
- **`guesty` shows Failed after restart (Windows):** the registered path is probably `/c/Users/...` instead of `C:\Users\...`, or it points at `.venv/bin/python`, which does not exist on Windows. Remove it (`claude mcp remove guesty -s user`) and re-run the Windows register block in section 3.
