...
claude mcp add --transport stdio guesty-official --scope user --env CLIENT_ID="$GUESTY_CLIENT_ID" --env CLIENT_SECRET="$GUESTY_CLIENT_SECRET" -- npx -y @guestyorg/sdk@0.2.0-alpha.1 mcp >/dev/null 2>&1 && echo "guesty-official registered ✅" || echo "guesty-official failed ❌ (run the same line without the >/dev/null part to see why)"
