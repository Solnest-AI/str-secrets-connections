...
- **B4 (register).** The connector file has the exact `claude mcp add` line (and its Windows `cygpath -w` form) for that server, plus the fan-out step that copies the credential from the root `.env` into the server's own `.env`.
