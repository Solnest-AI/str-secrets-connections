...
# A minimal PATH (just the OS dirs plus a Python) has no `claude` and no `uv`, only the
# system python3, which is exactly the desktop-app case this fallback exists for. Git Bash's
# /usr/bin ships no python, so the interpreter's own dir is added explicitly (on macOS this
